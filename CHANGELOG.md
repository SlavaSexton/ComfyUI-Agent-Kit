...
- **Four new models from the official Comfy source sweep** (the pre-release check that the v1.6.x cuts had skipped; run now against the live `Comfy-Org/workflow_templates` + blog):
  ...
  Counts refreshed everywhere (67 recipes, 545 templates, 149 models) including the cover and both breakdown charts, recomputed from the official template manifest. The Pixel Pro side was clean (export already triaged through its latest message).
