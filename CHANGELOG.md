...
published pro-color node pack (**ComfyUI-OCIO**), and a field guide to building nodes - local-first, for everyday
...
- **Positioning: local-first, for everyday users and experts.** README now states the kit scales from one-command generation to a professional VFX color pipeline (the OCIO pack), and routes agents to the OCIO pack for any color / VFX-color task.
