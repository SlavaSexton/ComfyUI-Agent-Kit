...
- **Per-model "mega-brain":** 75 prompt-recipe entries covering 68 named models, distilled from **official
...
- **Knows where each model runs:** a [full index](docs/MODEL_INDEX.md) of all 163 library models (recipe /
...
- **581-template library** (and **94 official Subgraph Blueprints**, reusable subgraph bricks) as the source of truth, plus **fetch any shared workflow by hash** and a **model
...
builds a compact lookup index so the agent can match any request to the right template. 581 templates (plus **94 official Subgraph Blueprints**, reusable subgraph bricks) span every
...
Covered today (75 recipe entries, 68 named models): FLUX.1/.2 + Kontext, Z-Image, Boogu, Mage-Flow, Qwen-Image/Edit, SDXL, SD1.5/3.5, HiDream,
...
`comfyui` skill (the 75-entry recipe brain + the docs). You still need a local ComfyUI on `http://127.0.0.1:8188`; the
