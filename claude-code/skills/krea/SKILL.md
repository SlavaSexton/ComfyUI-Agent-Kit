...
| Photoreal stills, opinionated photographic look, low VRAM | **FLUX.1 Krea Dev** (graph below) | 11.09 GB fp8, ordinary FLUX graph |
...
| Realtime or streaming video, interactive prompt changes | **not ComfyUI**, see the Realtime section | no ComfyUI path exists as of 2026-08-06 |
...
**11.09 GB measured**, ungated. The original `black-forest-labs/FLUX.1-Krea-dev` is gated, so reach for the
repack unless you need the reference weights. Licence flag: `flux-1-dev-non-commercial-license`.

## Krea Realtime 14B, and why it is not a ComfyUI job

Real, open, Apache-2.0, and **has no ComfyUI path**. Worth knowing before someone plans a graph around it.
...
- **The negative claim, and how it was checked:** a content search across GitHub for the checkpoint filename
  returns DiffSynth-Studio, LightX2V, `daydreamlive/scope`, research forks and their own repo. **No ComfyUI
  custom node.** The search was by file content across all of GitHub, not by repository name, because
  repository-name searches are exactly how this kit has been wrong before.
...
- **Fit on a 24 GB card:** the official single file is **26.61 GB measured**, so it does not fit. The community
  fp8 repack `6chan/krea-realtime-video-fp8` is **13.31 GB** in both `e4m3fn` and `e5m2`, which does. That repack
...
- **`lbouaraba/comfyui-krea2edit`** (410 stars, last push 2026-07-29). Instruction editing that holds a face,
...
- **Per-layer conditioning control, and it is a genuine two-horse choice.** The upstream is
  **`nova452/ComfyUI-ConditioningKrea2Rebalance`** (**477 stars**, pushed 2026-07-29), which introduced
...
| Looking for Krea Realtime nodes in ComfyUI | They do not exist. Use their server, DiffSynth-Studio or LightX2V |
| FLUX.1 Krea Dev download is gated | Use the ungated Comfy-Org repack, one 11.09 GB file |
